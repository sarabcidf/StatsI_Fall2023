...
# True b1 is 0.5
mean(par.est.ov[ , 1]) # r=0 : if x and z are not correlated with each other, it is safe to omit x
...
mean(par.est.ov[ , 11]) # r=0.99 # Bias becomes huge, and as coefficients
# become larger we also get more certainty to reject H0! So its largely problematic

# Note: multicollinearity is only an issue if r = 1 (perfectly correlated x1 and x2)
# there is really no reason to not control for highly correlated variables :) 
# there are no "degrees" of multicollinearity issues. Either you have it or you don't. 
# multicollinearity really only is a data problem, not so much a model or theory problem
...
# overfitting occurs when you mirror the data too closely; when you pick up
# the systematic signal in your sample, but also the random noise!
